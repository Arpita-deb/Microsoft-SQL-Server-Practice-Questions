...
-- 2. Show the Name of the customer, sales order id, date of order placed, quatity of products ordered per order, 
-- ListPrice of the products and total due for CustomerID 13590.

SELECT 
	CustomerName = P.[FirstName] + ' ' + P.MiddleName + ' ' + P.[LastName],
	H.[SalesOrderID],
	OrderDate = CAST([OrderDate] AS Date),
	OrderQtyPerOrder = SUM([OrderQty]) OVER(PARTITION BY H.[SalesOrderID]), 
	[UnitPrice],
	[TotalDue]
FROM 
...
-- 3. Show the first name and the email address of customer with Store name 'Bike World'.
...
SELECT Store,
[Total Due] = CAST(Subtotal + Tax + Freight AS NUMERIC(36,2)),
...
-- 7.Find the number of left racing socks ('Racing Socks, L') ordered by CompanyName(Store) 'Riding Cycles'.
...
-- 9. 10. Where did the racing socks go? List the product name and the CompanyName for all Customers who ordered ProductModel 'Racing Socks'.
...
/*
13. Show how many orders are in the following ranges (in $):
...
-- 14. Identify the three most important cities. Show the break down of top level product category against city.
...
--15. Show the total order value for each CountryRegion. List by value with the highest first.
...
/*
16.  Rank Customers based on Total Sales?
...
-- 18. List the name and addresses of companies containing the word 'Bike' (upper or lower case) and companies containing 
...
ORDER BY CASE 
            WHEN LOWER([Name]) LIKE '%bike%' THEN 1
            ELSE 2
         END,
         [Name] ASC;